...
def find_authors_nationalities():
    authors_without_nationality = Author.objects.exclude(nationality=None)
    result = []

    for author in authors_without_nationality:
...
def delete_review_by_id(review_id):
    ...
    r.delete()
    result = f'Review by {r.reviewer_name} was deleted'

    return result


def filter_authors_by_nationalities(nationality):
    authors_with_given_nationality = Author.objects.filter(nationality=nationality).order_by('first_name', 'last_name')

    result = []

    for a in authors_with_given_nationality:
